...
try:
    ...
    # 3. Sidebar Vertical Filter Controls (Region -> Area -> Territory -> Town)
    ...
    # B. Area Filter (Cascades vertically)
    ...
    # E. Additional Sales Officer Search
    if 'SO Name' in df.columns:
        search_so = st.sidebar.text_input("🔍 Search SO Name:")
        if search_so:
            df = df[df['SO Name'].str.contains(search_so, case=False, na=False)]

    # 4. Top KPI Executive Scorecard (Including Memos & Order counts)
    ...
    with col2:
        # Calculate unique memos based on combination of OrderDate & Outlet Code or invoice details if available
        # Default fallback to unique combination counts to define true product memos
    ...
    with col4:
        unique_brands = df['Brand'].nunique() if 'Brand' in df.columns else (df['Brand Name'].nunique() if 'Brand Name' in df.columns else 0)
        if unique_brands == 0:
            # Fallback tracking if column name is different
            for col in df.columns:
                if 'brand' in col.lower():
                    unique_brands = df[col].nunique()
                    break
        st.metric(label="Total Brands Distributed", value=f"{unique_brands}")
    ...
    with chart_col2:
        if 'SO Name' in df.columns:
            st.markdown("### 🏆 Top 10 Sales Officers (By Memo/Order Count)")
